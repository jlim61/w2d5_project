...
# -----------------------------DRIVER-----------------------------------------------------------------------------------------------
    def driver(self):
        print(f'\nGarage ticket price is currently ${self.ticket_price} for all day')
        ...
        while self.garage_status is True:
            ...
            enter_input = input('\nGarage Operator: Are you looking to [t]ake a ticket, [p]ay for parking, or [l]eave?:  ').lower()
...
# -----------------------------TAKING TICKET METHOD-----------------------------------------------------------------------------------------------
    def take_ticket(self):
        ...
        print(f'\n{self.parking_spaces[-1]} parking spaces available')
        if tickets[-1] > 0:
            self.current_tickets['ticket'] += 1
            self.tickets[-1] -= 1
            self.parking_spaces[-1] -= 1
            print(f"\nPlease grab your parking ticket\n\nYour ticket count is: {self.current_tickets['ticket']}\n")
            print(f'{self.parking_spaces[-1]} parking spaces remaining')
        else:
            print("\nNo parking spaces available left")
        ...
        if self.current_tickets['ticket'] > 0:
            while True:
                ...
                if pay_input.isdigit():
                    ...
                    self.tickets[-1] += 1
                    self.parking_spaces[-1] += 1
                    print(f"\nThank you for your payment of ${pay_input}! You have 15 minutes to leave garage.")
                    break
                else:
...
# -----------------------------LEAVE GARAGE METHOD-----------------------------------------------------------------------------------------------
    def leave_garage(self):
        if self.current_tickets['ticket'] > 0:
            print(f"\nYou still have {self.current_tickets['ticket']} ticket(s) to pay for.")
        else:
            print(f"\nThank you have a nice day!")
